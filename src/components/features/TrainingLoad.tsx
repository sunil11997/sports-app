...
const SESSION_TYPES = ["General Practice", "Intense Ground Work", "Tournament Match", "Gym / Strength", "Conditioning / Running", "Technical Skills", "Active Recovery"];
...
export function TrainingLoad({ store }: { store: any }) {
  ...
  const loadMetrics = useMemo(() => {
    ...
    return { acute: Math.round(acuteAvg), chronic: Math.round(chronicAvg), ratio: parseFloat(ratio.toFixed(2)), status };
  }, [playerLoadHistory]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Load Registry Updated", description: `Archived ${trainingLoadValue} load units for ${player?.name}.`, className: "bg-primary text-white font-black" });
    } catch (err) {
  ...
  const chartData = useMemo(() => {
    return playerLoadHistory.slice(-14).map(h => ({ name: format(parseISO(h.date), 'dd/MM'), load: h.load, rpe: h.rpe }));
  }, [playerLoadHistory]);
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20 max-w-7xl mx-auto">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest mt-1 flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-emerald-500" /> Sports Science Registry v4.3.25</p>
            </div>
          </div>
          <div className="flex flex-col w-full md:w-80 gap-3">
             <Select value={selectedPlayerId} onValueChange={setSelectedPlayerId}>
                <SelectTrigger className="h-14 rounded-2xl border-2 font-black bg-white shadow-sm"><SelectValue placeholder="Identify Athlete..." /></SelectTrigger>
                <SelectContent>{store.data.players.filter((p: any) => p.category === 'athlete').map((p: any) => (<SelectItem key={p.id} value={p.id}>{p.name} (Std {p.std})</SelectItem>))}</SelectContent>
             </Select>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 space-y-8">
          <Card className="border-2 rounded-[2.5rem] p-8 shadow-xl bg-white space-y-8">
            <div className="space-y-6">
               <div className="space-y-3"><label className="text-[10px] font-black text-primary uppercase tracking-widest ml-2">1. Session Context</label><Select value={sessionType} onValueChange={setSessionType}><SelectTrigger className="h-12 border-2 rounded-xl font-bold bg-muted/20"><SelectValue /></SelectTrigger><SelectContent>{SESSION_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></div>
               <div className="grid grid-cols-2 gap-4"><div className="space-y-3"><label className="text-[10px] font-black text-primary uppercase tracking-widest ml-2">2. Duration (Mins)</label><Input type="number" value={duration} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDuration(e.target.value)} className="h-12 rounded-xl border-2 font-black" /></div><div className="space-y-3"><label className="text-[10px] font-black text-primary uppercase tracking-widest ml-2">3. Date</label><Input type="date" value={date} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDate(e.target.value)} className="h-12 rounded-xl border-2 font-black" /></div></div>
            </div>
        ...
        <div className="lg:col-span-7 space-y-8">
           ...
           <Card className="border-2 rounded-[3rem] overflow-hidden bg-white shadow-xl flex flex-col min-h-[300px]"><CardHeader className="bg-primary p-6 text-white flex flex-row justify-between items-center"><div className="flex items-center gap-3"><Activity className="w-5 h-5 text-accent" /><CardTitle className="text-xs font-black uppercase tracking-widest">Load History Registry</CardTitle></div></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-muted/30 border-b"><tr className="h-12 text-[10px] font-black uppercase text-muted-foreground"><th className="px-8">Date</th><th className="px-4">Session Type</th><th className="px-4 text-center">RPE</th><th className="px-8 text-right">Unit Load</th></tr></thead><tbody className="divide-y">{playerLoadHistory.slice().reverse().slice(0, 5).map((h: LoadEntry, i: number) => (<tr key={i} className="h-16 hover:bg-muted/10 transition-colors"><td className="px-8 font-black text-xs text-primary uppercase">{format(parseISO(h.date), 'dd MMM yyyy')}</td><td className="px-4"><Badge variant="secondary" className="text-[9px] font-black uppercase px-3">{h.type}</Badge></td><td className="px-4 text-center font-bold">{h.rpe}</td><td className="px-8 text-right font-black text-primary">{h.load}</td></tr>))}{playerLoadHistory.length === 0 && (<tr><td colSpan={4} className="h-32 text-center text-[10px] font-black uppercase opacity-20">Awaiting registry entries</td></tr>)}</tbody></table></div></CardContent></Card>
        </div>
      </div>
    </div>
  );
