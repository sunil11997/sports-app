              ...
              {filteredPlayers.map((player: any) => {
                ...
                return (
                  <TableRow key={player.id} className={cn("border-b even:bg-muted/10 hover:bg-primary/5 transition-all h-16 group", isPulse && "animate-success-pulse")}>
                    ...
                    <TableCell className="border-r p-0"><Input type="number" step="0.1" placeholder="s" className="h-16 text-center text-sm font-black border-0 bg-transparent focus:bg-white rounded-none" value={current.shuttleRun || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(player.id, 'shuttleRun', e.target.value)} /></TableCell>
                    <TableCell className="border-r p-0"><Input type="number" placeholder="cm" className="h-16 text-center text-sm font-black border-0 bg-transparent focus:bg-white rounded-none" value={current.sitAndReach || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(player.id, 'sitAndReach', e.target.value)} /></TableCell>
                    <TableCell className="border-r p-0"><Input type="number" step="0.1" placeholder="s" className="h-16 text-center text-sm font-black border-0 bg-transparent focus:bg-white rounded-none" value={current.run50m || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(player.id, 'run50m', e.target.value)} /></TableCell>
                    <TableCell className="border-r p-0"><Input placeholder="m:s" className="h-16 text-center text-sm font-black border-0 bg-transparent focus:bg-white rounded-none" value={current.run600m || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(player.id, 'run600m', e.target.value)} /></TableCell>
                    <TableCell className="border-r p-0"><Input type="number" placeholder="reps" className="h-16 text-center text-sm font-black border-0 bg-transparent focus:bg-white rounded-none" value={current.sitUps || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(player.id, 'sitUps', e.target.value)} /></TableCell>
                    <TableCell className="border-r p-2 text-center bg-primary/5">
                      <div className="flex flex-col items-center gap-0.5">
                        ...
                        <Badge className="text-[8px] font-black uppercase px-2 py-0 rounded-md shadow-sm bg-primary text-white">
