...
const translations = {
  ...
  Marathi: {
    ...
    offlineStatus: "ऑफलाईन मोड (लोकल)",
    onlineStatus: "क्लाउड कनेक्टेड"
  }
};
...
export default function WaghambaApp() {
  ...
  const classSummaries = useMemo(() => {
    ...
    for (let i = 1; i <= 12; i++) summary[i.toString()] = { total: 0, boys: 0, girls: 0 };
        ...
        <div className="max-w-xl w-full text-center space-y-10 relative z-10">
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="w-52 h-52 bg-white rounded-full flex items-center justify-center mx-auto border-4 border-accent shadow-2xl overflow-hidden active-scale group p-4">
              <Image src={LOGO_INAPP} alt="App Logo" width={200} height={200} priority unoptimized className="object-contain w-full h-full group-hover:scale-110 transition-transform duration-700" />
            </div>
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground font-black tracking-[0.4em] text-[10px] uppercase">{t.tribalLogoHint}</p>
            </div>
          </div>
          <div className="pt-8 animate-in fade-in slide-in-from-bottom-12 duration-700 delay-200 space-y-4">
            <Button onClick={handleStartHub} className="group w-full bg-primary hover:bg-primary/90 text-white font-black text-xl h-20 rounded-[1.5rem] shadow-xl transition-all active-scale">
              {t.enter} <ArrowRight className="ml-3 w-6 h-6 group-hover:translate-x-1 transition-transform" />
            </Button>
            {isInstallable && (
              <Button onClick={installApp} variant="outline" className="w-full border-2 border-primary/10 bg-white text-primary font-black text-xs h-12 rounded-xl active-scale uppercase tracking-widest">
                <Smartphone className="mr-2 w-4 h-4" /> {t.installApp}
              </Button>
            )}
            <p className="text-muted-foreground/30 text-[9px] font-black uppercase tracking-[0.5em] pt-8">{t.institutionalStack}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-muted/20 flex flex-col items-center justify-center p-6">
        <div className="max-w-4xl w-full space-y-12 text-center animate-in fade-in zoom-in-95 duration-500">
          <div className="space-y-3">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-primary uppercase tracking-tight">{t.selectHub}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <button onClick={() => handleSectionSelect('sports')} className="bg-white border-2 border-transparent rounded-[2.5rem] p-10 text-left transition-all hover:border-accent hover:shadow-2xl active-scale shadow-lg group relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-muted-foreground font-medium text-sm leading-relaxed opacity-80">{t.sportsHubDesc}</p>
                <div className="mt-8 flex items-center gap-2 text-accent font-black uppercase text-[10px] tracking-widest">Open Portal <ChevronRight className="w-4 h-4" /></div>
              </div>
            </button>
            <button onClick={() => handleSectionSelect('general')} className="bg-white border-2 border-transparent rounded-[2.5rem] p-10 text-left transition-all hover:border-primary/20 hover:shadow-2xl active-scale shadow-lg group relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-muted-foreground font-medium text-sm leading-relaxed opacity-80">{t.studentRegistryDesc}</p>
                <div className="mt-8 flex items-center gap-2 text-primary font-black uppercase text-[10px] tracking-widest">Access Registry <ChevronRight className="w-4 h-4" /></div>
              </div>
            </button>
          </div>
        </div>
      ...
      {isInstallable && showInstallBanner && (
        <div className="bg-primary text-white p-3 flex justify-between items-center animate-in slide-in-from-top duration-500 sticky top-0 z-[60] shadow-lg">
          ...
          <div className="flex items-center gap-2">
            <Button onClick={installApp} size="sm" className="bg-white text-primary font-black text-[10px] h-8 px-5 rounded-full hover:bg-white/90 shadow-lg active-scale">INSTALL</Button>
            <Button variant="ghost" size="icon" onClick={() => setShowInstallBanner(false)} className="text-white/60 hover:text-white h-8 w-8 rounded-full"><X className="w-4 h-4" /></Button>
          </div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => handleTabChange('home')}>
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-muted-foreground uppercase mt-0.5 tracking-widest truncate max-w-[150px]">{profile.schoolName}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Button variant="outline" size="sm" onClick={() => setSelectedSection(null)} className="text-[9px] font-black uppercase bg-white rounded-full h-10 md:h-8 px-4 border-border hover:bg-muted">
              {t.switchHub}
            </Button>
          </div>
        </div>
